            ...
            <div className="mb-8">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center rounded-xl border border-slate-200 overflow-hidden bg-slate-50">
                  <button onClick={() => setQuantity((q) => Math.max(1, q - 1))} className="w-11 h-11 flex items-center justify-center text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition-all">
                    <Minus size={15} />
                  </button>
                  <span className="w-10 text-center text-slate-900 font-bold text-lg" style={{ fontFamily: 'var(--font-mono)' }}>{quantity}</span>
                  <button onClick={() => setQuantity((q) => q + 1)} className="w-11 h-11 flex items-center justify-center text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition-all">
                    <Plus size={15} />
                  </button>
                </div>
            ...
            <div className="hidden md:flex gap-3 mb-10">
              <motion.button
                ...
                className={`flex-1 flex items-center justify-center gap-3 py-4 rounded-2xl text-white font-bold text-lg transition-all duration-200 ${
                  addedToCart ? 'bg-emerald-500' : 'bg-violet-600 hover:bg-violet-700 shadow-violet'
                }`}
              >
              ...
              <motion.button
                ...
                className={`w-14 h-14 flex items-center justify-center rounded-2xl border-2 transition-all duration-200 ${
                  fav ? 'bg-violet-600 border-violet-600 text-white' : 'border-slate-200 text-slate-400 hover:border-violet-400 hover:text-violet-500'
                }`}
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-slate-100 rounded-2xl overflow-hidden border border-slate-100">
                  {product.specs.map((spec) => (
                    <div key={spec.label} className="flex items-center justify-between px-4 py-3 bg-white">
                      ...
                      <span className="text-slate-900 text-sm font-semibold" style={{ fontFamily: 'var(--font-mono)' }}>{spec.value}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 p-4 bg-white/95 border-t border-slate-200 shadow-lg" style={{ backdropFilter: 'blur(12px)' }}>
        <div className="flex items-center gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-slate-900 font-bold text-lg" style={{ fontFamily: 'var(--font-mono)' }}>₪{(product.price * quantity).toLocaleString()}</p>
          </div>
          <motion.button
            onClick={handleAddToCart}
            whileTap={{ scale: 0.97 }}
            className={`flex items-center gap-2 px-6 py-3.5 rounded-2xl text-white font-bold transition-all ${addedToCart ? 'bg-emerald-500' : 'bg-violet-600 shadow-violet'}`}
          >
            <AnimatePresence mode="wait">
              {addedToCart ? (
